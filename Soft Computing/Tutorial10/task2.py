...
def get_mnist():
    ...
    class IrisDataset(T.utils.data.Dataset):
        ...
        def __getitem__(self, idx):
            if T.is_tensor(idx):
                idx = idx.tolist()
            preds = T.from_numpy(self.data[idx, 0:4]).to(device)
            spcs = T.from_numpy(self.data[idx, 4:]).to(device)
            # to actually use for training must convert to T.long:
            # spcs = T.tensor(self.data[idx, 4:]), dtype=T.long).to(device)
